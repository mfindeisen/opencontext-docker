...
Mesh.prototype = {
	...
	httpRequest: function(start, end, load, error, abort, type) {
		...
		// r.setRequestHeader('Access-Control-Allow-Methods', 'GET,POST,OPTIONS');
		r.onload = function(){
			switch (this.status){
				case 206:
                			load.bind(this)();
...
function readyNode(context, node) {
	...
	m.coords[n] = new Float32Array(node.buffer, 0, nv);
	m.indices[n] = new Uint16Array(node.buffer, nv*m.vsize, nf*3);
